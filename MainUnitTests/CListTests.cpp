...
#include <sstream>
#include <algorithm>
#include "..//CustomContainers/CList.h"
...
namespace CustomContainersTests
{
	...
	public:
		...
		template<typename T>
		void EqualToVector(CList<T> list, const std::vector<T> & v)
		{
			Assert::AreEqual(list.Size(), v.size());
			const typename CList<T>::CListNode* cur = list.cbegin();
			for (const T& x : v)
			{
		...
		TEST_METHOD(SwapTest1)
		{
			...
			CList<int> l1{ 8, 4, 9, 15, 22 };
			std::vector<int> v2 = { 9, 15, 7, 21 };
			CList<int> l2{ 9, 15, 7, 21 };
			l1.Swap(l2);
